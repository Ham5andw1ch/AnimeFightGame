...
int main(int argc, char** argv)
{
    if(argc < 5)
    {
        fprintf(stderr, "Usage: %s [animage sprite] [animage palette] [Num Frames] [width]\n", argv[0]);
        return 1;
    }
...
//          printf("%i %i %i %i\n", pixels[i], pixels[i+1], pixels[i+2], pixels[i+3]);
    ...
    SDL_Window* win = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, spr->width/num + num, spr->height, 0);
